...
namespace cs31
{
    ...
    // COMPLETED: let the human player play
    ...
    // COMPLETED: let the computer player play
    // when the amount is zero, the Die should be rolled for random play
    // when the amount is not zero, we are trying to cheat for testing
    // purposes.  in that case, this method should force a specific roll
    // amount.
    ...
    // COMPLETED: determine the game outcome at this moment
    ...
    // COMPLETED: determine if the game is over
    ...
    bool Pig::isGameOver()
    {
        if (mBoard.getComputerTotal() >= 30 || mBoard.getHumanTotal() >= 30)
            mBoard.setGameOver(true);
        
        if (determineGameOutcome() == COMPUTERWONGAME)
