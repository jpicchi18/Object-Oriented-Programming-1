...
namespace cs31
{
    ...
    // COMPLETED: update the human's score but only if it is their turn and the game isn't over
    void Board::setHumanScore( int score )
    {
        if (mIsHumanTurn == true && mGameOver == false) {
            mHumanScore = score;
        }
    }
    
    // COMPLETED: trivial accessor
    int  Board::getHumanScore() const
    {
        return( mHumanScore );
    }
    
    // COMPLETED: update the computer's score but only if it is their turn and the game isn't over
    void Board::setComputerScore( int score )
    {
        if (mIsHumanTurn == false && mGameOver == false)
            mComputerScore = score;
    }
    
    // TODO COMPLETED: trivial accessor
    ...
    // COMPLETED: update the human's total but only if it is their turn and the game isn't over
    ...
    // COMPLETED: update the computer's total but only if it is their turn and the game isn't over
